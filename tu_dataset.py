...
class TUDatasetExt(InMemoryDataset):
    url = 'https://data.pyg.org/datasets'
    ...
    def process(self):
        result = read_tu_data(self.raw_dir, self.name)
        print(f"read_tu_data returned {len(result)} values.")

        if len(result) == 2:
            self.data, self.slices = result
        elif len(result) == 3:
            self.data, self.slices, additional_info = result
        else:
