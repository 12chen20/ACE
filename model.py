...
from torch.nn import Linear, BatchNorm1d, Sequential, ReLU, Parameter, Dropout
...
class CausalGCN(torch.nn.Module):
    def __init__(self, num_features, num_classes, args, gfn=False, collapse=False, residual=False,
                 res_branch="BNConvReLU", global_pool="sum", dropout=0, edge_norm=True):
        ...
        self.conv_feat = GATConv(hidden_in, hidden, heads=1, concat=True)
        self.bns_conv = torch.nn.ModuleList()
        self.convs = torch.nn.ModuleList()

        for i in range(num_conv_layers):
            self.bns_conv.append(BatchNorm1d(hidden))
            self.convs.append(GATConv(hidden, hidden, heads=1, concat=True))

        self.node_att_gat = GATConv(hidden, 2, heads=2, concat=False)
        self.edge_att_gat = GATConv(hidden * 2, 2, heads=2, concat=False)

        self.bnc = BatchNorm1d(hidden)
        self.bno = BatchNorm1d(hidden)
        self.context_convs = GATConv(hidden, hidden, heads=2, concat=False)
        self.objects_convs = GATConv(hidden, hidden, heads=2, concat=False)
    ...
    def forward(self, data, eval_random=True):
        ...
        if self.without_node_attention:
            ...
            node_att = self.node_att_gat(x, edge_index)
            node_att = F.softmax(node_att, dim=-1)
        ...
        if self.without_edge_attention:
            ...
            edge_att = self.edge_att_gat(edge_rep, edge_index)
            edge_att = F.softmax(edge_att, dim=-1)
...
class CausalGIN(torch.nn.Module):
    def __init__(self, num_features, num_classes, args, gfn=False, edge_norm=True):
    ...
    def random_readout_layer(self, xc, xo, eval_random):
        ...
        lambda_values = [i * 0.05 for i in range(21)]
        lambda_val = random.choice(lambda_values)

        if self.args.cat_or_add == "cat":
            x = torch.cat(((1 - lambda_val) * xc[random_idx], lambda_val * xo), dim=1)
        else:
            x = xc[random_idx] + xo
...
class CausalGAT(torch.nn.Module):
    def __init__(self, num_features, num_classes, args, head=4, dropout=0.1, edge_norm=True, gfn=False):
        ...
        self.fc_num = args.fc_num
        self.bn_feat = BatchNorm1d(hidden_in)
        self.conv_feat = GATConv(hidden_in, hidden // head, heads=head, concat=True, dropout=dropout)
        self.bns_conv = torch.nn.ModuleList()
        self.convs = torch.nn.ModuleList()
        self.without_node_attention = args.without_node_attention
        self.without_edge_attention = args.without_edge_attention
        ...
        for i in range(num_conv_layers):
            self.bns_conv.append(BatchNorm1d(hidden))
            self.convs.append(GATConv(hidden, hidden // head, heads=head, concat=True, dropout=dropout))
        ...
        self.bnc = BatchNorm1d(hidden)
        self.bno = BatchNorm1d(hidden)
        self.context_convs = GATConv(hidden, hidden // head, heads=head, concat=True, dropout=dropout)
        self.objects_convs = GATConv(hidden, hidden // head, heads=head, concat=True, dropout=dropout)
    ...
    def random_readout_layer(self, xc, xo, eval_random):
        ...
        if self.args.cat_or_add == "cat":
            ...
            x = lambda_val * xc[random_idx] + (1 - lambda_val) * xo
...
class GINNet(torch.nn.Module):
    def __init__(self, num_features, num_classes, hidden,
                 num_fc_layers=2, num_conv_layers=3, dropout=0):
...
class GATNet(torch.nn.Module):
    def __init__(self, num_features, num_classes, hidden, head=4,
                 num_fc_layers=2, num_conv_layers=3, dropout=0.2):
